  ...
  var server = net.createServer(options, function (connection) {
    ...
    client.on(COMMAND_MAPS[COMMANDS.GET_JOB], function (msg) {
      ...
      if (reqJobQ.length === 0) {
        ...
        var job = reqJobQ.shift();
        exeJobs[this.id] = job;
        logger.debug('Agent : job(id) = %s, exeJobs(keys) = %s', job.id, Object.keys(exeJobs));
        this.send({
    ...
    client.on(COMMAND_MAPS[COMMANDS.COMPLETE_JOB], function (msg) {
      ...
      if (job) {
        ...
        delete exeJobs[this.id];
        job.timer = null;
        logger.debug('Agent: exeJobs(keys) = %j', Object.keys(exeJobs));
      }
    });
...
Agent.prototype.stop = function (cb) {
  ...
  var closeClients = function (clients, frmpsr, msgpsr, cb) {
  ...
  closeClients(this.clients, this.frmpsr, this.msgpsr, function () {
