      ...
      frmpsr.expand(data, function (frame) {
        ...
        switch (req.cmd) {
          ...
          case COMMANDS.SUBMIT_JOB:
            ...
            job.timer = null;
            logger.debug('Agent : command = SUBMIT_JOB, req = %j, job = %j', req, job);
            if (job.timeout > 0) {
          ...
          case COMMANDS.REGIST_WORKER:
            var id = connectionID(con);
            logger.debug('Agent : command = REGIST_WORKER, id = %s', id);
          ...
          case COMMANDS.GET_JOB:
            if (reqJobQ.length === 0) {
              ...
              logger.debug('Agent : command = GET_JOB, job(id) = %s, exeJobs(keys) = %s', job.id, Object.keys(exeJobs));
              con.write(frmpsr.shrink([
                msgpsr.pack({
            ...
            break;
          case COMMANDS.COMPLETE_JOB:
          case COMMANDS.FAILED_JOB:
            var id = connectionID(con);
            var job = exeJobs[id];
            if (job) {
...
Agent.prototype.stop = function (callback) {
  ...
  var closeClients = function (clients, frmpsr, msgpsr, callback) {
    ...
    clients.forEach(function (connections) {
      Object.keys(connections).forEach(function (key) {
        ...
        connection.end(frmpsr.shrink([msgpsr.pack(req)]));
  ...
  closeClients([this.clients, this.workers], this.frmpsr, this.msgpsr, function () {
