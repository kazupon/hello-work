      ...
      var server = net.createServer(options, function (connection) {
        ...
        client.on(COMMAND_MAPS[COMMANDS.GET_JOB], function (msg) {
          ...
          if (req_job_queue.length === 0) {
            ...
            var job = req_job_queue.shift();
            exe_jobs[this.id] = job;
            logger.debug('Agent : job(id) = %s, exe_jobs(keys) = %s', job.id, Object.keys(exe_jobs));
            this.send({
        ...
        client.on(COMMAND_MAPS[COMMANDS.COMPLETE_JOB], function (msg) {
          ...
          if (job) {
            ...
            job.timer = null;
            logger.debug('Agent: exe_jobs(keys) = %j', Object.keys(exe_jobs));
          }
        });
