...
Base.prototype.connect = function (/*options, callback*/) {
  ...
  if (this.socket) {
    ...
    process.nextTick(this.emit.bind(this, 'connect', err));
    return;
  }
  ...
  socket.connect(port, host, function () {
    ...
    var listener = function (data) {
      ...
      frames.forEach(function (frame) {
        ...
        if (res.cmd === common.CMD_JOIN) {
        ...
        } else if (res.cmd === common.CMD_LEAVE) {
          ...
          socket.once('close', function (err) {
            ...
            process.nextTick(self.emit.bind(self, 'disconnect'));
          });
        } else { // other
          self.onCommand(res);
        }
    ...
    this.write(frmpsr.shrink([self.msg.pack(req)]));
...
Base.prototype.disconnect = function (callback) {
  ...
  if (!this.socket) {
    ...
    process.nextTick(this.emit.bind(this, 'disconnect', err));
    return;
  }
  ...
  socket.end(this.frmpsr.shrink([this.msg.pack(req)]));
