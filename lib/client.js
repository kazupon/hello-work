/**
 * client.js
 * @author kazuya kawaguchi <kawakazu80@gmail.com>
 */

//
// import(s)
//

var util = require('util');
var net = require('net');
var common = require('./common');
var COMMANDS = common.COMMANDS;
var Job = common.Job;
var Base = require('./base').Base;


//
// clsss(es)
//

/**
 * helloworld client class.
 * @class Client
 * @constructor
 */
function Client () {

  Base.call(this);
  this.type = 'client';
  this.jobs = {};

  var self = this;
  this.onCommand = function (res) {
    //console.log('Client : onCommand -> %j', res);
    switch (res.cmd) {
      case COMMANDS.CREATED_JOB:
        var job = new Job();
        job.id = res.job.id;
        job.ns = res.job.ns;
        job.func = res.job.func;
        self.jobs[job.id] = job;
        self.callback(job);
        break;
      case COMMANDS.COMPLETE_JOB:
        var job = self.jobs[res.job.id];
        //console.log('Client : COMPLETE_JOB -> %j', job);
        job.emit('complete', {
          result: res.job.result
        });
        delete self.jobs[res.job.id];
        break;
      case COMMANDS.FAILED_JOB:
        var job = self.jobs[res.job.id];
        //console.log('Client : FAILED_JOB -> %j', job);
        job.soonEmit('fail', {
          err: new Error(res.job.result.message)
        });
        delete self.jobs[res.job.id];
        break;
      case COMMANDS.TIMEOUT_JOB:
        var job = self.jobs[res.job.id];
        console.log('Client : TIMEOUT_JOB -> %j', job);
        job.emit('timeout', {
          code: res.job.code
        });
        delete self.jobs[res.job.id];
        break;
      default:
        self.soonEmit('error', new Error('Unknown Command Error'));
        break;
    }
  };

}

util.inherits(Client, Base);


/**
 * Queueing a job.
 * @method do
 * @param {Object} [params] a parameters.
 * @param {String} [params.ns] a namespace.
 * @param {String} [params.func] a function name.
 * @param {Object} [params.args] a function arguments.
 * @param {Number} [params.timeout] a timeout milisecond.
 * @param {Function} callback a callback function.
 * @throws {Error}
 */
Client.prototype.do = function (params, callback) {

  // check connect.
  if (!this.socket) {
    throw new Error('Not Connect');
  }

  //check  parameter.
  if (!params || !params.func || (typeof(params.func) !== 'string') ||
      (params.func === '') || !callback || (typeof(callback) !== 'function')) {
    throw new Error('Invalid Parameter');
  }
  var ns;
  if (!params.ns || (typeof(params.ns) !== 'string')) {
    ns = '/';
  } else {
    var match = common.REGEX_NS.exec(params.ns);
    ns = (!match ? '/' : match[1]);
  }

  this.callback = callback;

  var func = params.func;
  var args = (params.args && (typeof(params.args) === 'object')) ? params.args : null;
  var timeout = (params.timeout && (typeof(params.timeout) === 'number') && (params.timeout > 0)) ? params.timeout : 0;
  var req = {
    cmd: COMMANDS.SUBMIT_JOB,
    params: {
      ns: ns,
      func: func,
      args: args,
      timeout: timeout
    }
  };
  this.socket.write(this.frmpsr.shrink([this.msgpsr.pack(req)]));

};


//
// export(s)
//

module.exports = {
  Client: Client
};

